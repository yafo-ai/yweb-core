...
- 在 **`async def` 事件循环线程** 上直接执行同步 IO 会阻塞事件循环，故引入：
  ...
  - **`run_db(func)`**：用 Starlette `run_in_threadpool` 把整块同步 DB 逻辑放到线程池执行。

外部项目（如 `y-sso-system`）中大量 `async def` 路由被迫写成：

```python
apps = await run_db(lambda: application_model.query.filter_by(is_active=True).all())
```
...
同时保留 **`run_db()`** 作为兼容与细粒度控制手段。

### 1.3 非目标（本方案不承诺）

- 将全栈改为 **原生 AsyncSession + asyncpg**（与现有 `scoped_session`、软删除重写、历史版本等耦合深，另立项目级迁移）。
- 让 **`cls.query.session.execute(...)`** 在 `async def` 里“自动 await”（Session 级 API 仍建议 `run_db` 或 `def` 路由）。
...
| 文件 | 说明 |
|------|------|
| [assets/hybrid_query_refactor_before_after.png](assets/hybrid_query_refactor_before_after.png) | **async 路由**：重构前（拦截 / `run_db`）与重构后（`await` + HybridQuery）对照 |
...
| `async def` 中写 DB | 必须 `run_db(lambda: ...)` 或改 `def` 路由 | 可对链式查询 **`await .all()` / `.first()` 等**；复杂块仍可用 `run_db` |
...
### 3.3 `async def` + `run_db`（当前推荐补救）

```mermaid
flowchart TD
    A[async def route] --> B["await run_db(lambda: ...)"]
...
> **本质**：不是把数据库变成异步 IO，而是把 **“执行这条 Query”** 从事件循环线程挪到线程池——与 `run_db` 同族，只是粒度变成 **终端方法**。
...
| 规则 | 内容 |
|------|------|
| **R1** | **线程池只服务于「在事件循环线程上执行 Query 终端」**。实现上：终端对象实现 `__await__`，内部调用 **`run_in_threadpool`（与现有 `run_db` 同源）** 执行「构建好的同步 `Query` 的一次执行」。 |
...
写库、Session 级操作、类方法封装查询：**仍用** `await run_db(lambda: ...)` 或 **`def` 路由**（§5、§8 已说明原因）；本方案 **不** 为省这几行去动整个 `CoreModel`。
...
| **不改** | `core_model.py` 中 `save` / `bulk_*` / 历史等实例与类方法（继续 `run_db` 或 `def`） |
| **保留** | `run_db` 原样可用 |
...
### 5.0 图例（async def 路由下「能否无 `run_db`」）

| 标记 | 含义 |
|------|------|
| **✅ 计划支持** | 通过 **`await Model.query....<终端>`**（HybridQuery）在实现后应能安全用于 `async def`，底层线程池执行同步 Query。 |
| **⚠️ 条件/额外工作** | 部分路径可支持，或存在懒加载等边界，需在实现与测试中单独约定。 |
| **❌ 不支持** | 非 `Model.query` 链终端；在 **`async def` 中直接调用仍会阻塞事件循环**（或触发 `get_session` 的 async 安全检测）。须 **`def` 路由**、**`await run_db(...)`** 或 **`with_db_session` / `@with_db_session`**。 |
...
|------|---------------------|------|
| `Model.query.session` | **❌ 不支持「自动 await」** | 必须返回 **真实 `Session`**，以兼容 `core_model` / 缓存等处的 `cls.query.session.execute` / `commit` / `merge`。在 **`async def` 里调用 `session.execute` 等仍会阻塞**，须 `run_db` 或 `def` 路由。 |
...
| 成员 | async 无 `run_db` | 主要 DB 路径（源码依据） | 说明 |
|------|-------------------|-------------------------|------|
...
1. **HybridQuery 只接管一条入口**  
   ...
   实现上等于：在「执行这条 Query」的瞬间把同步调用 **丢进线程池**（与 `run_db` 同族）。
...
3. **即便技术上能包，语义也更难保证**  
   ...
   - 若对每个实例方法都做 `await` 包装，本质上仍是 **每次调用 `run_in_threadpool`**，与 **`await run_db(lambda: user.save(True))`** 相同量级，但会 **分散隐式线程跳转**，排错更难。

因此：§5.2 / §5.3 标 **❌** 的含义是 **「本 HybridQuery 方案不承诺自动 await」**，不是「永远不能用在 async 应用里」——用 **`def` 路由、`run_db`、或 `with_db_session`** 即可。

#### 5.2.2 有没有「更好」的支持方式？—— 可选演进（按投入与风险排序）

| 方案 | 做法概要 | 优点 | 缺点 / 风险 |
|------|----------|------|-------------|
| **A. 架构约定（推荐作为默认）** | **写路径** 用 `def` 路由或 `run_db` 包一整段业务；**读路径** 用 `await Model.query...` | 与 FastAPI 官方建议一致；事务清晰；改动面最小 | 开发者需记住「读 / 写」分流 |
| **B. 继续用好现有工具** | `run_db`、`with_db_session`、`@with_db_session` | 已存在、语义明确 | 样板代码比「裸 await」略多 |
| **C. Service / 用例级封装** | 在 Service 层提供 `async def create_user(...)`，内部 **`await run_db(lambda: ...)`** 一次包全段 | 路由层干净；事务在一个 lambda 内 | 需维护 Service API |
...
- **写模型、Session 级操作** 更合理的「更好方案」通常是 **A + B + C**（约定 + 现有工具 + 用例级 `run_db`），而不是强行让 `save`/`delete` 等每一个实例方法都变成可 `await` 的魔法（除非走 **F** 做全栈 async ORM）。

---

### 5.3 `CoreModel` 类方法（非 `Model.query` 显式链）

| 方法 | async 无 `run_db` | 主要 DB 路径 | 说明 |
|------|-------------------|-------------|------|
...
| `get` | **❌** | `cls.query.filter_by(id=...)` 再 `count`/`first` | **注意**：这是 **类方法封装**，不是 `await Model.query...`；本方案 **不自动** 为 `Model.get(id)` 生成 await 版，async 中请 `run_db(lambda: Model.get(id))` 或先 `await Model.query.filter_by(id=id).first()`。 |
...
| `paginate`（classmethod） | **⚠️** | `Query` 分支：`query.count` / `offset`/`limit`；`Select` 分支：`cls.query.session.execute` / `scalar` | **Query** 子路径可由 HybridQuery 覆盖；**Select** 子路径仍走 Session，async 须整体 `run_db` 或拆查询为 `await Model.query...`。 |
...
**特殊**：`__getattribute__(id)` 在 pending 时可能 **`session.flush()`** — **❌** 在 async 中访问未落库实例的 `id` 可能触发同步 IO，宜在 `run_db` 内完成 `save`/flush。
...
flowchart TD
    ...
    B -->|get/save/bulk/历史/session| E["await run_db 或 def 路由"]
    E --> F[不支持自动 await]
```

- **✅ 计划由 HybridQuery 覆盖**：仅 **`Model.query` 起点** 的 SQLAlchemy **Query 链 + 终端**（见 §5.1）。
- **❌ 其余 `CoreModel` / `BaseModel` API**：在 **`async def` 中须 `run_db`、改为 `def` 路由，或 `with_db_session`**；本方案 **不** 承诺为每个类方法生成 async 变体。
...
| 测试 | 覆盖：`def` 路由行为不变；`async def` + `await` 终端；`query.session.*`；与 `run_db` 共存 |
...
| `asyncio.gather(await q1.all(), await q2.all(), ...)` 并发多终端 | 同一请求 `request_id` → 同一 `Session`；多个线程池线程并发读写同一 `Session` **非线程安全**（SA 明确禁止） | **文档明确禁止**在同一请求内并发触发终端 await；若确有并发需求，为每个分支用独立 `await run_db(lambda: with db_session_scope(request_id=...): ...)` 拿到独立 Session；或改串行 `await`。 |
...
| `await Model.query....all()` 与 `run_db(lambda: Model.query....all())` 混用 | `run_db` 与 HybridQuery 终端都走 `run_in_threadpool`；在同一 `request_id` 下串行即正确，并发则回到第 3 条 | 约定：同一请求内 **串行** 使用；禁止在 `asyncio.gather` 里混跑多个 DB 终端/`run_db`。 |
...
- HybridQuery 终端执行完成后 Session **没被清理**（清理留给中间件），但**可能被其他 `run_db` / 终端**触发 commit / expire。再加上请求末尾 `on_request_end()` 会 `session_scope.remove()`，之前 `await` 拿到的实例如果被异步任务持有到请求结束之后（如 `BackgroundTasks`、`asyncio.create_task`），再访问就是 `DetachedInstanceError`。
...
- **对策**（任选）：
  ...
  2. 严格路由：`async def` 路由里只允许 **1 次** 终端 await / `run_db`，禁止「await 查 → 再 `run_db` 写」串行在同一 handler；
...
- **对策**：
  ...
  3. 若不想扩大改动面，文档明示「async 中访问 `dynamic` 关系必须 `run_db`」。
...
- yweb 的 `transaction_manager` 是**同步上下文管理器**：`with transaction_manager.begin(): ...`。它和 HybridQuery 的组合存在概念错配：
  ...
  - HybridQuery **终端默认不 commit**；写事务仍需 `run_db(lambda: ...)` 或 `with_db_session`，在一个 callable 内包住 `begin → 查 → 改 → commit`。
- **对策**：明文「**任何写事务都不要跨 `await`**」；写路径一律用 `await run_db(lambda: ...)` 或 `@with_db_session`，把 `transaction_manager.begin()` 完整包进 lambda。
...
| 坑点 | 说明 / 对策 |
|------|-------------|
| **大结果集 / 流式** | HybridQuery 不提供 stream API；`.yield_per()` 的 `__iter__` 被禁用后，大数据导出改走 `def` 路由或 `run_db(lambda: for chunk in q.yield_per(500): ...)`。 |
...
| **async 里调同步 DB** | 框架 **不会** 自动帮你包；需自行 `run_in_threadpool` / `asyncio.to_thread`（yweb 的 `run_db` 即此类）。 |
...
| **仍非「真异步」** | 与 `run_db`、Django `a*` 底层类似：**吞吐靠多线程**，不是 asyncpg 那种 **单线程多路复用**；高并发下线程数与池大小仍是瓶颈。 |
...
| **用例级 `await run_db(lambda: ...)`** | 一次包 **多步** `save`+`query`，**一次**线程跳转，语义清晰。 |
| **HybridQuery：只包 `Model.query` 链终端** | **窄切口**、易测；与「细粒度读、粗粒度写」习惯一致。 |
| **Django 式：为高频 API 增加 `asave` / `aget`…** | 体验最好，**维护成本最高**；适合长期投入的核心框架团队。 |
| **CoreModel 全局自动线程池** | **理论上能覆盖所有调用**，但综合 **性能、Session 正确性、双重进池、可维护性**，一般 **劣于** 上三行；仅当产品强制「全部 async 路由且禁止 run_db」时可再评估 **有限范围**（例如仅包装 `get_session` 一级并严格避免嵌套）。 |
...
| 你的优先级 | 更优路径 |
|------------|----------|
| **最少改动、最快稳定** | **`def` 路由** 做 DB；`async def` 只做纯异步 IO；必要时 **`run_db` 包块**。 |
| **async 路由里读库体验好** | **HybridQuery**（本文主方案）+ 写仍 **`run_db` / `def`**。 |
...
| 维度 | 说明 |
|------|------|
| **做法** | 业务 **不直接** 依赖 `CoreModel.query` / `save`；只依赖 **`IUserRepository`** / **`UnitOfWork`** 接口。实现类可为 **`SyncUserRepository`** 或 **`AsyncUserRepository`**（或内部用 `run_db` / AsyncSession）。FastAPI `Depends` 注入具体实现。 |
...
#### 8.8.2 底层工具：用 `asyncer` / `anyio.to_thread` 替代 / 补充 `run_db`

| 维度 | 说明 |
|------|------|
| **做法** | `run_db` 现在是 `starlette.concurrency.run_in_threadpool(func, *args, **kwargs)` 的薄封装。可考虑换 / 补 `asyncer.asyncify(func, abandon_on_cancel=False)` 或直接用 `anyio.to_thread.run_sync(func, ..., abandon_on_cancel=False)`：前者是 FastAPI 作者的同胞库，后者是 Starlette 底层。 |
...
| **代价** | 多一层抽象；需要明确每条 `run_db` 调用的取消语义（CancelledError 的处理策略）。 |

> 结论：**默认 `run_db` 不需要换**；但文档里应点名：真正自己写 `loop.run_in_executor(executor, ...)` 时 **必须** 自行 `contextvars.copy_context()`，否则 `request_id` / 当前用户 / 软删除开关都会丢（参见 §7.2.2 第 2 条）。
...
| 优先级 | 推荐路径 |
|--------|----------|
| 最少改动 | `def` 路由 / `run_db`（现状） |
| async 读体验 | **HybridQuery**（§4.5）+ `run_db` 写 |
